const Hero = () => {
  ...
  return (
    <section className="flex max-md:flex-col max-md:gap-16 items-center justify-center py-5 px-5 pt-9">
        ...
        <div id="title" className="font-bold text-7xl max-sm:text-5xl text-center confirm_selection "><h1>XMOONEY</h1></div>
        ...
        <div className="space-y-6">
            ...
            <div id="ctas" className="grid gap-3 grid-cols-2 w-fit mx-auto">
                {
                    links.map((link)=>(
                        <a className="border-2 border-white max-sm:text-[10px] bg-gray-900 text-center py-3 px-5 uppercase rounded-lg" key={link.name} href={link.link}>{link.name}</a>
                    ))
                }
            </div>
        </div>
